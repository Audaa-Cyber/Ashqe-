...
-- Billing settlement lock. Serializes payment-intent settlement and
-- subscription replacement so concurrent verification cannot create two active subscriptions.
...
begin
  ...
  update public.ashqe_payment_intents
    set status='paid', paid_at=v_start, tx_hash=p_tx_hash,
  ...
  update public.ashqe_subscriptions
    set status='expired', updated_at=v_start
    where user_id=p_user_id and status='active';

  insert into public.ashqe_subscriptions
    (user_id,plan_id,status,current_period_start,current_period_end,payment_intent_id,chain,token,tx_hash)
  values
    (p_user_id,v_plan_id,'active',v_start,v_end,p_intent_id,p_chain,p_token,p_tx_hash);
  return true;
