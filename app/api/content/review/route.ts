...
type ReviewResult = {
  score: number
  flags: string[]
  recommendation: "keep" | "review" | "rewrite"
  rewrite?: string
}
...
export async function POST(request: Request) {
  ...
  const recommendation: ReviewResult["recommendation"] =
    heuristic.recommendation === "rewrite" || heuristic.recommendation === "review" || heuristic.recommendation === "keep"
  ...
  if (process.env.OPENROUTER_API_KEY) {
    try {
      ...
      const parsed = JSON.parse(modelText) as Partial<ReviewResult>
      const modelRecommendation: ReviewResult["recommendation"] =
        parsed.recommendation === "rewrite" || parsed.recommendation === "review" || parsed.recommendation === "keep"
          ? parsed.recommendation
          : "review"
      if (typeof parsed.score === "number" && Array.isArray(parsed.flags)) {
        result = {
          score: Math.round(Math.max(0, Math.min(100, parsed.score))),
          flags: parsed.flags.filter((flag): flag is string => typeof flag === "string").slice(0, 8),
  ...
  const { error } = await supabase.from("ashqe_content_reviews").insert({
    user_id: user.id,
    content_type: String(body.contentType || "post"),
    source_text: text,
