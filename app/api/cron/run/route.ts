...
function cronFieldMatches(field:string,value:number){
  ...
  return field.split(",").some(part=>{
    if(part.includes("/")){const [base,stepText]=part.split("/");const step=Number(stepText);if(!step)return false;const start=base==="*" ? 0 : Number(base);return value>=start&&(value-start)%step===0}
  ...
  for(const job of jobs??[]){
    ...
    try{
      if(job.action_type==="research"){
        ...
        const authz=await authorizeAutonomousAction(admin,job.user_id,job.action_type,{targetId:job.config?.targetId,recipientOptedIn:Boolean(job.config?.recipientOptedIn),aiReplyApproved:Boolean(job.config?.aiReplyApproved)})
        ...
        if(job.destination==="telegram"){
          const {data:tg}=await admin.from("ashqe_telegram_connections").select("chat_id").eq("user_id",job.user_id).maybeSingle()
          if(tg?.chat_id)await sendTelegram(tg.chat_id,"ASHQE executed "+job.action_type+":\n"+clean+"\nhttps://x.com/"+conn.x_username+"/status/"+posted.id)
        }
      }
