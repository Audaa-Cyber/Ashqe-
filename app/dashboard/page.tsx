...
export default async function DashboardPage() {
  ...
  const { data: connection } = await supabase
    .from("x_connections")
    .select("x_username, x_name, x_avatar_url, recent_posts")
    .eq("user_id", user.id)
    .maybeSingle()
  ...
  const { data: style } = await supabase
    .from("style_profiles")
    .select(
      "tone, length_pref, rhythm, topics, signature_phrases, do_list, dont_list, summary, posts_analyzed, updated_at",
    )
    .eq("user_id", user.id)
    .maybeSingle()
  ...
  // Most recent session, plus its messages — create one if none exists yet
  ...
  return (
    <DashboardShell
      ...
      connection={{
        ...
        avatarUrl: connection.x_avatar_url,
      }}
      style={style ?? null}
      drafts={drafts ?? []}
