...
function withinHours(policy: ExecutionPolicy) {
...
export async function authorizeAutonomousAction(
  ...
  opts: { targetId?: string; recipientOptedIn?: boolean; aiReplyApproved?: boolean } = {},
) {
  ...
  else if (!withinHours(policy)) { status = "blocked"; reason = "outside_allowed_hours" }
